...
//         log('Init - storedStaffId:', storedStaffId);
...
//         if (storedStaffId && storedSocietyId) {
...
//           const chatsResponse = await axios.get(`http://localhost:3000/api/chat/user/${storedStaffId}`);
//           log('Chats response:', chatsResponse.data);
//           setChats(chatsResponse.data.map(chat => ({ ...chat, chatId: chat._id })));

//           const requestsResponse = await axios.get(`http://localhost:3000/api/chat/staff/requests/${storedStaffId}`);
//           log('Requests response:', requestsResponse.data);
//           setPendingRequests(requestsResponse.data);
//         } else {
//           setError('Missing staffId or societyId. Please log in again.');
//           log('Init error: Missing staffId or societyId');
//         }
//       } catch (error) {
...
//       axios.get(`http://localhost:3000/api/chat/staff/requests/${staffId}`).then(res => {
//         log('Updated pending requests:', res.data);
//         setPendingRequests(res.data);
//       }).catch(err => log('Fetch requests error:', err.message));
//     });

//     socket.on('requestAccepted', async ({ chatId }) => {
//       log('Received requestAccepted:', { chatId });
//       try {
//         const chatsResponse = await axios.get(`http://localhost:3000/api/chat/user/${staffId}`);
//         log('Updated chats:', chatsResponse.data);
//         setChats(chatsResponse.data.map(chat => ({ ...chat, chatId: chat._id })));
//         const newChat = chatsResponse.data.find(chat => chat._id === chatId);
//         if (newChat) {
//           log('Selecting new chat from requestAccepted:', chatId);
//           selectChat({ ...newChat, chatId: newChat._id });
//         }
//       } catch (err) {
//         log('Fetch chats error:', err.message);
//       }
//     });
...
//       setErrorModalMessage('Society ID not found. Please log in again.');
//       setShowErrorModal(true);
//       return;
//     }
//     try {
//       log('Searching users with query:', searchQuery, 'societyId:', societyId);
//       const response = await axios.get(`http://localhost:3000/api/chat/search?query=${encodeURIComponent(searchQuery)}&societyId=${societyId}&userType=staff`);
//       log('Search response:', response.data);
//       setSearchResults(response.data);
//     } catch (error) {
//       log('Search error:', error.message);
//       setErrorModalMessage('Failed to search users. Please try again.');
//       setShowErrorModal(true);
//       setSearchResults([]);
//     }
//   };
...
//       setErrorModalMessage('You cannot send a chat request to yourself.');
//       setShowErrorModal(true);
//       return;
//     }
...
//       setErrorModalMessage('A chat request is already pending with this user.');
//       setShowErrorModal(true);
//       return;
//     }
...
//       setErrorModalMessage('You already have an active chat with this user.');
//       setShowErrorModal(true);
//       return;
//     }
...
//       setErrorModalMessage('Failed to send chat request.');
//       setShowErrorModal(true);
//     }
//   };
...
//       setErrorModalMessage('Failed to accept chat request.');
//       setShowErrorModal(true);
//     }
//   };
...
//       setErrorModalMessage('Failed to reject chat request.');
//       setShowErrorModal(true);
//     }
//   };
...
//       setErrorModalMessage('Failed to delete chat.');
//       setShowErrorModal(true);
//     }
//   };
...
//           setErrorModalMessage('Failed to send message: ' + response.error);
//           setShowErrorModal(true);
//           setMessages(prev => ({
//             ...prev,
...
//       setErrorModalMessage('Failed to send message. Please check your connection.');
//       setShowErrorModal(true);
//     }
//   };
...
//       setErrorModalMessage('Failed to fetch messages.');
//       setShowErrorModal(true);
//     }
//   };
...
//     if (!manager) {
//       setErrorModalMessage('Manager not found.');
//       setShowErrorModal(true);
//       return;
//     }
//     openRequestModal(manager);
//   };
...
//     return (
//       <View style={styles.container}>
//         <Text style={styles.errorText}>Loading staff data...</Text>
//       </View>
//     );
//   }
...
//             <Text style={styles.headerText}>Staff Chat</Text>
...
export default function StaffChatScreen() {
  ...
  useEffect(() => {
    const init = async () => {
      try {
        ...
        if (storedStaffId && storedUserId && storedSocietyId) {
          setStaffId(storedStaffId);
          setUserId(storedUserId);
          ...
          try {
            ...
            log('Requests fetch error:', requestError.response?.data?.error || requestError.message);
            setError(requestError.response?.data?.error || 'Failed to load chat requests. Please try again or contact support.');
          }
        } else {
          setError('Missing staffId, userId, or societyId. Please log in again.');
          log('Init error: Missing staffId, userId, or societyId');
        }
      } catch (error) {
        log('Initialization error:', error.message);
        setError('Failed to initialize chat. Please try again or contact support.');
      }
    };
    init();
    ...
    socket.on('requestAccepted', async ({ chatId }) => {
      ...
      try {
        ...
        log('Updated chats:', chatsResponse.data);
        setChats(chatsResponse.data.map(chat => ({ ...chat, chatId: chat._id })));
        const newChat = chatsResponse.data.find(chat => chat._id.toString() === chatId);
        if (newChat) {
          log('Selecting new chat from requestAccepted:', chatId);
          selectChat({ ...newChat, chatId: newChat._id });
        } else {
          log('New chat not found in response:', chatId);
          setErrorModalMessage('Chat created but not found. Please refresh.');
          setShowErrorModal(true);
        }
      } catch (err) {
        log('Fetch chats error:', err.message);
        setErrorModalMessage('Failed to fetch new chat. Please try again.');
        setShowErrorModal(true);
      }
    });
  ...
  const sendChatRequest = async () => {
    try {
      ...
      log('Request error:', error.message);
      setErrorModalMessage('Failed to send chat request. Please try again.');
      setShowErrorModal(true);
    }
  };
