...
//       console.log('Fetched staff events:', events.map(e => ({ _id: e._id, title: e.title, status: e.status, organizer: e.organizer?._id, dateTime: e.dateTime, image: e.image ? 'base64' : null })));
//       res.json(events);
//     } catch (error) {
//       console.error('Fetch staff events error:', error);
//       res.status(500).json({ message: 'Failed to fetch staff events', error: error.message });
//     }
//   });
